...
class Reactor(BaseOperator):
    ...
    def __init__(self, acc, gas_for_approve = 0.25, gas_for_execute=0.4):
        ...
        self.deposit_contracts = {
            'ETH': self.get_contract(ZK_REACTOR_CONTRACTS["rfETH"], REACTOR_ABI),
            'USDC': self.get_contract(ZK_REACTOR_CONTRACTS["rfUSDC"], REACTOR_ABI)
        } 
        self.collateral_contract = self.get_contract(ZK_REACTOR_CONTRACTS["collateral"], REACTOR_ABI)
    ...
    def get_deposit_amount(self, token):
        ...
        amount = self.deposit_contracts[token].functions.balanceOf(self.acc.address).call()
    ...
    def withdraw(self, token):
        ...
        if amount > self.withdrawl_minimum[token]:
            ...
            tx = self.deposit_contracts[token].functions.redeem(amount)
